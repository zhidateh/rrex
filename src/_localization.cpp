...
public:
  ...
  void callback_imu(const sensor_msgs::Imu::ConstPtr& msg_imu)
  {
    ...
    if(init_step < 100){
      ROS_INFO("Initialising IMU to get initial position, loading status %d %%", init_step);
      ...
      imuData.hdg   = roundto3DP(_imuData.hdg - init_imuData.hdg);
      imuData.pitch = roundto3DP(_imuData.pitch - init_imuData.pitch);
      imuData.roll  = roundto3DP(_imuData.roll - init_imuData.roll);
  ...
  void callback_encoder(const rrex::Encoder::ConstPtr& msg_encoder)
  {
    ...
    if(init_step >= 100) fusionAlgorithm();
  }
  ...
  void fusionAlgorithm()
  {
    ...
    hdgs.push_back(encoderData.hdg);

    //50Hz
    if(dt > fps){
      start_time = std::chrono::steady_clock::now();
      
      y_vl << roundto3DP(std::accumulate(vel_ls.begin(),vel_ls.end(),0.0)/(double)vel_ls.size());
      y_vr << roundto3DP(std::accumulate(vel_rs.begin(),vel_rs.end(),0.0)/(double)vel_rs.size());
      y_hdg << roundto3DP(std::accumulate(hdgs.begin(),hdgs.end(),0.0)/(double)hdgs.size());
      vel_ls.clear();
      vel_rs.clear();
      hdgs.clear();
      ...
      double _vel_l =  kf_vl->state().transpose()(0);
      double _vel_r =  kf_vr->state().transpose()(0);
      double _vel = 0.5 * (_vel_l + _vel_r);
      
      double _hdg = kf_hdg->state().transpose()(0);
      ...
      state_x += _vel*cos(_hdg)*dt;
      state_y += _vel*sin(_hdg)*dt;
      
      ROS_INFO("%s: %.2f", "State x  : "  , state_x);
      ROS_INFO("%s: %.2f", "State y  : "  , state_y);
      ROS_INFO("%s: %.2f", "Heading  : "  , _hdg);
      ROS_INFO("%s: %.2f", "Velocity : "  , _vel);
      ROS_INFO("%s: %.2f", "Left vel : "  , _vel_l);
      ROS_INFO("%s: %.2f", "Right vel: "  , _vel_r);
      ROS_INFO("----------------------------------");
      
      rrex::FusedPose fused_pose_msg;
      fused_pose_msg.x      = state_x;
      fused_pose_msg.y      = state_y;
      fused_pose_msg.hdg    = _hdg;
